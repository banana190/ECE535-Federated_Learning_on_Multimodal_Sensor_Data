            ...
            for modalities in combo[dataset]:
                ...
                for k in schemes:
                    ...
                    for rep in range(N_REPS):
                        rep_file = os.path.join("D:\ML\iotdi22-mmfl-main\\results", dataset.lower(
                        ...
                        for rep in range(N_REPS):
                            if "ablation" not in k:
                                rep_file = os.path.join(
                                    "D:\ML\iotdi22-mmfl-main\\results", dataset.lower(), ae, f"{modalities[0]}_{modalities[1]}", schemes[k][1], "results.txt")
                            else:
                                rep_file = os.path.join(
                                    "D:\ML\iotdi22-mmfl-main\\results", dataset.lower(), "ablation", f"{modalities[0]}_{modalities[1]}", schemes[k][1], "results.txt")
                            data = np.loadtxt(rep_file, delimiter=",")
                            x_all = data[:, 0]
